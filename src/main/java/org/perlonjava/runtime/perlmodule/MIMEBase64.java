...
public class MIMEBase64 extends PerlModuleBase {
    ...
    public static void initialize() {
        ...
        base64.defineExport("EXPORT", "encode_base64", "decode_base64", "encode_base64url", "decode_base64url");
        ...
        try {
            ...
            base64.registerMethod("encode_base64url", null);
            base64.registerMethod("decode_base64url", null);
    ...
    public static RuntimeList encode_base64url(RuntimeArray args, int ctx) {
        if (args.size() > 1) {
            throw new IllegalStateException("Bad number of arguments for encode_base64url");
        }
        RuntimeScalar str = args.get(0);
        ...
        byte[] inputBytes = input.getBytes(StandardCharsets.ISO_8859_1);
        String encoded = Base64Util.encodeUrl(inputBytes);

        return new RuntimeScalar(encoded).getList();
    }

    public static RuntimeList decode_base64url(RuntimeArray args, int ctx) {
        if (args.size() > 1) {
            throw new IllegalStateException("Bad number of arguments for decode_base64url");
        }
        RuntimeScalar input = args.get(0);
        String encoded = input.toString();
