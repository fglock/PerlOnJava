...
public class Attributes extends PerlModuleBase {
    ...
    /**
     * Attach a runtime-compiled anonymous sub body to the CODE reference that
     * already participated in compile-time attribute dispatch.
     */
    public static RuntimeScalar adoptCompileTimeCodeRef(
            RuntimeScalar compiledRef, RuntimeScalar compileTimeRef) {
        if (compiledRef.type != CODE || compileTimeRef.type != CODE) {
            return compiledRef;
        }
        ...
        if (placeholder.codeObject instanceof RuntimeCode nested) {
            nested.__SUB__ = compileTimeRef;
        }
        if (placeholder.codeObject != null) {
            try {
                var field = placeholder.codeObject.getClass().getDeclaredField("__SUB__");
                field.setAccessible(true);
                field.set(placeholder.codeObject, compileTimeRef);
            } catch (NoSuchFieldException ignored) {
                // Java-backed/native Perl subs do not expose a generated __SUB__ field.
            } catch (IllegalAccessException e) {
                throw new PerlCompilerException(
                        "Unable to attach compile-time anonymous sub: " + e.getMessage());
            }
        }
        return compileTimeRef;
    }
