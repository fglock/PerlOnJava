...
// Shift Right (>>)
     ...
     */
    public RuntimeScalar transliterate(String search, String replace, String modifiers) {
        // Get the input string that will undergo transliteration
        String input = this.toString();

        // Determine if /r modifier is used, which indicates the original string should be unchanged
        boolean returnOriginal = modifiers.contains("r");

        // If /c (complement) is used, modify the search list to include all characters not in the original search list
        if (modifiers.contains("c")) {
            search = complementSearchList(search);
        }

        // Create a mapping array for the transliteration
        char[] translationMap = new char[256];
        // boolean[] usedChars = new boolean[256];  // Track characters used in replacement to handle /s (squash) modifier
        ...
        boolean lastCharWasReplaced = false;
        for (int i = 0; i < input.length(); i++) {
            char originalChar = input.charAt(i);
            char translatedChar = translationMap[originalChar];

            // Handle /d (delete) modifier: skip characters that map to '\0'
            if (translatedChar == '\0' && modifiers.contains("d")) {
                continue;
            }

            // Handle /s (squash) modifier: skip duplicate replaced characters
            if (modifiers.contains("s") && translatedChar == result.charAt(result.length() - 1)) {
                continue;
            }

            // Append the translated character unless it's a duplicate being squashed
            if (translatedChar != '\0' || !modifiers.contains("d")) {
        ...
        // Return either the modified string or the original based on the /r modifier
        if (returnOriginal) {
    ...
    /**
     * Complements the search list by including all characters not present in the original search string.
     *
     * @param search The original search string.
     * @return The complemented search string.
     */
    private String complementSearchList(String search) {
