...
public class ModuleOperators {
    ...
    private static RuntimeBase doFile(RuntimeScalar runtimeScalar, boolean setINC, boolean isRequire, int ctx) {
        ...
        // the file being required/do'd, and filters installed inside the
        // required file must not leak back to the caller.  Real Perl scopes
        // filters per compilation unit (PL_compiling / PL_rsfp_filters);
        // we mirror that by snapshotting the outer state and starting with
        // a clean slate, restored unconditionally in the finally below.
