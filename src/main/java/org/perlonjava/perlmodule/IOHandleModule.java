...
/**
 * Java backend for IO::Handle module.
 * Provides low-level I/O operations that can't be implemented in pure Perl.
 */
public class IOHandleModule extends PerlModuleBase {

    // Buffer for ungetc functionality
    private static final java.util.Map<Integer, Integer> ungetcBuffers = new java.util.concurrent.ConcurrentHashMap<>();

    public IOHandleModule() {
    ...
    public static void IOHandleInit() {
        ...
        try {
            ioHandle.registerMethod("_ungetc", "*$");
            ...
            System.err.println("Warning: Missing IO::Handle method: " + e.getMessage());
        }
    }
     ...
     */
    public static RuntimeList _ungetc(RuntimeArray args, int ctx) {
        if (args.size() != 2) {
            throw new IllegalStateException("Bad number of arguments for _ungetc");
        }

        RuntimeIO fh = RuntimeIO.getRuntimeIO(args.get(0));
        int ord = args.get(1).getInt();
        ...
        // Store the character in ungetc buffer
        int fd = fh.fileno().getInt();
        if (fd >= 0) {
            ungetcBuffers.put(fd, ord);
