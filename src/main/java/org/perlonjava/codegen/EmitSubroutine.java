...
public class EmitSubroutine {
    ...
    // Thread-local recursion depth counter to detect infinite recursion during compilation
    private static final ThreadLocal<Integer> compilationDepth = ThreadLocal.withInitial(() -> 0);
    private static final int MAX_COMPILATION_DEPTH = 50;  // Conservative limit to detect runaway refactoring
    ...
    public static void emitSubroutine(EmitterContext ctx, SubroutineNode node) {
        // Check for infinite recursion during compilation
        int depth = compilationDepth.get();
        if (depth > MAX_COMPILATION_DEPTH) {
            String errorMsg = String.format(
                "Compilation depth exceeded %d (currently at %d) when compiling subroutine '%s' at position %d.\n" +
                "This suggests excessive nesting or a refactoring issue where closures are creating more closures indefinitely.\n" +
                "Common causes:\n" +
                "  - Too many captured variables in closures (causes large constructors)\n" +
                "  - Refactoring creating chunks that are still too large\n" +
                "  - Genuine deeply nested code structure",
                MAX_COMPILATION_DEPTH, depth, node.name != null ? node.name : "<anon>", node.tokenIndex);
            throw new RuntimeException(errorMsg);
        }
        compilationDepth.set(depth + 1);
        try {
            emitSubroutineInternal(ctx, node);
        } finally {
            compilationDepth.set(depth);
        }
    }

    private static void emitSubroutineInternal(EmitterContext ctx, SubroutineNode node) {
        ...
        // Scan the AST to find which variables are actually used
        // This prevents capturing unnecessary variables which can cause "method too large" errors
        Set<String> usedVariableNames = findUsedVariables(node.block);
        int originalCount = visibleVariables.size();

        // Temporarily disable filtering to debug - log what we would do
        ctx.logDebug("Visible variables: " + visibleVariables.values().stream()
                .map(e -> e.name()).collect(java.util.stream.Collectors.joining(", ")));
        ...
        // TODO: Currently disabled because variable indices in the AST need remapping
        // The visitor correctly identifies used variables, but the generated bytecode
        // still references variables by their original slot indices which become invalid
        // after filtering. Need to either:
        // 1. Remap all variable indices in the AST after filtering, OR
        // 2. Use variable names for lookups instead of slot indices
        boolean enableFiltering = false;  // Disabled - needs index remapping
        if (enableFiltering && !isPackageSub) {
            visibleVariables.entrySet().removeIf(entry -> {
                String varName = entry.getValue().name();
                boolean isUsed = usedVariableNames.contains(varName);
                if (!isUsed) {
                    ctx.logDebug("Filtering unused variable: " + varName);
                }
                return !isUsed;
            });
        }

        int filteredCount = visibleVariables.size();
        if (enableFiltering && originalCount > filteredCount) {
            ctx.logDebug(String.format("Variable capture: reduced from %d to %d variables (saved %d)",
    ...
    /**
     * Scans the AST to find which variables are actually referenced in the subroutine.
     * This allows closures to capture only the variables they use, avoiding massive constructors
     * that can trigger "method too large" errors.
     *
     * @param block The subroutine body to scan
     * @return Set of variable names (with sigils) that are actually used
     */
    private static Set<String> findUsedVariables(Node block) {
        VariableReferenceCollector collector = new VariableReferenceCollector();
        if (block != null) {
            block.accept(collector);
        }
        return collector.getReferencedVariables();
    }

    /**
     * Visitor that collects all variable references in an AST.
     * Uses the Visitor pattern to ensure all node types are handled.
     */
    private static class VariableReferenceCollector implements org.perlonjava.astvisitor.Visitor {
        ...
        // Check if a node is a variable reference and collect it
        private void checkVariableReference(Node node) {
            if (node instanceof OperatorNode opNode) {
                if (opNode.operator.matches("[$@%&]") && opNode.operand instanceof IdentifierNode idNode) {
        ...
        public void visit(SubroutineNode node) {
            // Don't descend into nested subroutines - they have their own scope
        }
        ...
        // Default implementations for other node types - just traverse if possible
        ...
        @Override public void visit(LabelNode node) {
            // Labels don't contain variable references themselves
        }
        @Override public void visit(For1Node node) {
            // Visit the list being iterated over
            if (node.list != null) {
            ...
            // Visit the continue block if present
            if (node.continueBlock != null) {
                node.continueBlock.accept(this);
            }
            // Note: node.variable is the loop variable itself (like $_ or $x in "for my $x")
            // which is declared by the loop, so we don't need to capture it from outer scope
        }
    ...
    /**
     * Recursively collects all variable names referenced in an AST node.
     * DEPRECATED: Use VariableReferenceCollector visitor instead.
     */
    private static void collectVariableReferences(Node node, Set<String> varNames) {
