...
public class EmitterMethodCreator implements Opcodes {
    ...
    private static byte[] getBytecodeInternal(EmitterContext ctx, Node ast, boolean useTryCatch, boolean disableFrames) {
        ...
        try {
            ...
            // Parse the generated bytecode to find the largest method's code length.
            // If the code comes from a string eval (fileName = "(eval N)") AND the largest
            // method exceeds JPERL_MAX_METHOD_BYTES (default 9 000), trigger interpreter
            // fallback rather than letting the C1 JIT fail under register pressure.
            //
            // We restrict the threshold to eval-generated code because:
            //   1. The C1 JIT failures are caused by Sub::Quote / Moo string-eval'd methods.
            //   2. Regular Perl files can also have large methods (e.g. unit test bodies up to
            //      16 KB) that compile and run correctly on the JVM backend but fail on the
            //      interpreter due to known parity gaps (local, tied, etc.).
            //   3. String-eval'd code that triggers this threshold is exactly the Sub::Quote
            //      path where the interpreter fallback is already known to work.
            //
            // This path also produces JPERL_BYTECODE_SIZE_DEBUG output when that env var is set.
            String bytecodeSizeDebug = System.getenv("JPERL_BYTECODE_SIZE_DEBUG");
            String maxMethodBytesEnv = System.getenv("JPERL_MAX_METHOD_BYTES");
            // Default: 9 000 bytes — catches all 7 Sub::Quote methods (smallest: 9 043 B).
            // Set to 0 to disable, or Integer.MAX_VALUE to disable the threshold entirely.
            int maxMethodBytesThreshold = (maxMethodBytesEnv != null && !maxMethodBytesEnv.isEmpty())
                    ? Integer.parseInt(maxMethodBytesEnv) : 9_000;
            boolean sizeDebugEnabled = bytecodeSizeDebug != null && !bytecodeSizeDebug.isEmpty();

            // Determine if the source code is from a string eval ("(eval N)" filename).
            // Only eval-generated methods have the C1 failure problem we're trying to fix.
            ...
            // Only parse when debug is on (always parse for debug), OR when the source is eval
            // code and the class is large enough to potentially exceed the threshold.
            // Fast path: if classData is smaller than the threshold, no single method can exceed it.
            if (sizeDebugEnabled || (isEvalCode && classData.length > maxMethodBytesThreshold)) {
                try {
                    if (sizeDebugEnabled) {
