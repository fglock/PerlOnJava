...
BEGIN {
    # PerlOnJava: regex (?(DEFINE)...) groups aren't supported, so
    # force the legacy 5.8-style (??{...}) regex code path in
...
# PerlOnJava: route XS-only accessors through pure-Perl implementation.
{
    require Config;
...

{
    ...
    # PerlOnJava: changed from `my %METAS` to `our %METAS` so the
    # reachability walker (which only seeds from package globals)
    # can see the metaclass cache and prevent premature DESTROY of
    # metaclasses during Class::MOP / Moose bootstrap. With `my`,
    # the hash leaves MyVarCleanupStack once Class::MOP.pm finishes
    # loading and is reachable only via closure captures the walker
    # doesn't follow.
    our %METAS;
