...
# The upstream XS layer only names generated coderefs for diagnostics.  The
# generated accessors themselves are pure Perl, and PerlOnJava's bundled
# Sub::Util implementation provides the portable equivalent when available.
use Sub::Util qw();

sub set_sub_name {
  my ($code, $package, $name, $stash_name) = @_;
  return unless $code && $package && $name && $stash_name;
  Sub::Util::set_subname($stash_name, $code)
