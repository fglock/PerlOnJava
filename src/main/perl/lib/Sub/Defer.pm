...
# This is a PerlOnJava-tuned version of Sub::Defer
...
sub defer_sub {
  ...
  # PerlOnJava modification: Don't use weak refs to avoid reachability walker timeout
  # This bundled version uses strong refs instead, with END block cleanup
  $deferred_info->[4] = $deferred;
  $DEFERRED{$deferred} = $deferred_info;
  return $deferred;
}
...
sub CLONE {
  ...
  foreach my $deferred (keys %DEFERRED) {
    my $info = $DEFERRED{$deferred};
    next unless $info;
    # Clear weak references to avoid OOM during cleanup
    undef $DEFERRED{$deferred};
  }
}

# PerlOnJava modification: Always auto-clear %DEFERRED on exit to avoid timeout
END {
  %DEFERRED = ();
}
