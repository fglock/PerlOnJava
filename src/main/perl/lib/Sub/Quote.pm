...
# This is a PerlOnJava-tuned version of Sub::Quote
# Based on CPAN version 2.006009
# Modifications:
# - CLONE method checks PERLONJAVA_SKIP_CLONE_CLEANUP environment variable
#   to avoid OOM-triggering cleanup during tests on PerlOnJava
# - When PERLONJAVA_SKIP_CLONE_CLEANUP is set, CLONE becomes a no-op
#   This prevents the reachability walker's GC forcing mechanism from
#   causing OutOfMemoryError during program exit/cleanup
...
sub quote_sub {
  ...
  my $unquoted;
  # PerlOnJava modification: Don't use weak refs to avoid reachability walker timeout
  $quoted_info->{unquoted} = \$unquoted;
  if ($options->{no_defer}) {
    my $fake = \my $var;
    ...
    $quoted_info->{deferred} = $deferred;
    $QUOTED{$deferred} = $quoted_info;
    return $deferred;
  }
}
...
sub CLONE {
  # PerlOnJava modification: Always skip CLONE cleanup to avoid OOM during cleanup
  # This bundled version is specifically for PerlOnJava where CLONE cleanup causes OOM
  # No weak refs are used, so CLONE cleanup is unnecessary
  return;
}

# PerlOnJava modification: Always auto-clear %QUOTED on exit to avoid timeout
END {
  %QUOTED = ();
}
