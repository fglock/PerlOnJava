...
{
    # PerlOnJava: replaced upstream's recursive (?&...) / (??{...})
    # regex parser with a hand-rolled character-level parser.
    # PerlOnJava's regex engine doesn't yet support either of those.

    sub _atom_re { qr{ [\w:\.]+ }x }

    # Find the position of the matching ] for a [ at index $start.
    ...
    # Split a type string at top-level `|` (not inside [ ]).
    sub _split_top_union {
        my ($s) = @_;
        my @parts;
        my $depth = 0;
        my $start = 0;
        ...
        for (my $i = 0; $i < $len; $i++) {
            ...
            if ($c eq '[') { $depth++; }
                ...
                my $piece = substr($s, $start, $i - $start);
                $piece =~ s/^\s+|\s+$//g;
                push @parts, $piece;
                $start = $i + 1;
            }
        }
        my $piece = substr($s, $start);
        $piece =~ s/^\s+|\s+$//g;
        push @parts, $piece if length $piece;
        return @parts;
    }
    ...
    sub _parse_parameterized_type_constraint {
        ...
        if ($s =~ /^([\w:\.]+)\s*\[/) {
            my $base = $1;
            my $bracket = index($s, '[', length($base));
            return ($base) if $bracket < 0;
            my $end = _match_bracket($s, $bracket);
            return ($base) if $end < 0;
            my $inner = substr($s, $bracket + 1, $end - $bracket - 1);
            $inner =~ s/^\s+|\s+$//g;
            return ($base, $inner);
        }
        if ($s =~ /^([\w:\.]+)$/) {
    ...
    sub _detect_parameterized_type_constraint {
        my $s = $_[0];
        $s =~ s/^\s+|\s+$//g;
        return 0 unless $s =~ /^([\w:\.]+)\s*\[(.*)\]$/s;
        my $bracket = index($s, '[');
        my $end = _match_bracket($s, $bracket);
        return 0 if $end < 0;
        return $end == length($s) - 1 ? 1 : 0;
    }

    sub _parse_type_constraint_union {
        my $given = shift;
        $given =~ s/^\s+|\s+$//g;
        return _split_top_union($given);
    }

    sub _detect_type_constraint_union {
        my $s = $_[0];
        $s =~ s/^\s+|\s+$//g;
        my @parts = _split_top_union($s);
        return @parts >= 2 ? 1 : 0;
    }
}
