...
sub import {
    ...
    foreach my $f (@_) {
        my $fno = $fields->{$f};
        # PerlOnJava: simplified - just record the field, skip validation.
        # The original checks for field reuse and module reloading are
        # not needed since PerlOnJava doesn't support restricted hashes.
        unless ($fno) {
            $fields->{$f} = $next;
