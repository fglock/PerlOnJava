...
$ABI_VERSION = 2;
require Exporter;
# PerlOnJava: no XSLoader needed - methods registered by XMLLibXML.initialize()
@ISA = qw(Exporter);
...
#-------------------------------------------------------------------------#
# PerlOnJava: version check removed (Java stubs return matching versions)
...
# No-op DESTROY: XS normally provides this; needed so SUPER::DESTROY works
# in subclasses (XML::LibXML::Element calls $self->SUPER::DESTROY)
sub DESTROY {}
...
# Document constructor: XML::LibXML::Document->new($version, $encoding)
# Delegates directly to the Java XS docCreateDocument primitive (registered as
# XML::LibXML::Document::createDocument) to avoid infinite recursion with the
# Perl-level XML::LibXML::createDocument wrapper.
sub new {
    my $class = shift;
    my ($version, $encoding) = @_;
    # Call the XS 'createDocument' registered in this package directly
    return $class->createDocument($version, $encoding);
}
...
sub setDocumentElement {
    ...
    # Adopt node from foreign documents to avoid WRONG_DOCUMENT_ERR
    if (defined $element && defined $element->ownerDocument
        && !$doc->isSameNode($element->ownerDocument)) {
        $element = $doc->adoptNode($element);
    }
...
# PerlOnJava: these are XS stubs — no native libxml2 callback wiring needed
