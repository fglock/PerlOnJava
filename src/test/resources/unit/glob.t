...
my $nested_path = File::Spec->catfile('subdir', 'nested.txt');
open my $fh, '>', $nested_path or die $!;
close $fh;
...
subtest 'Absolute paths' => sub {
    ...
    my $normalized_tmpdir = File::Spec->canonpath($tmpdir);
    my $first_file = File::Spec->canonpath($abs_files[0]);

    # Check if the file starts with the temp directory path
    ok(index($first_file, $normalized_tmpdir) == 0, 'Returns absolute paths');
};
...
subtest 'No recursion into subdirectories' => sub {
    ...
    # Use File::Spec for cross-platform path comparison
    my $expected_path = File::Spec->catfile('subdir', 'nested.txt');
    is($subdir_txt[0], $expected_path, 'Found nested file');
};

subtest 'Special characters in filenames' => sub {
    # Create files with special characters that are valid on all platforms
    my @special_files = ('test[1].txt', 'test{a}.txt');
    my $special_count = 2;

    # On non-Windows systems, also test asterisk in filename
    if ($^O ne 'MSWin32') {
        push @special_files, 'test*.txt';
        $special_count = 3;
    }
    ...
    # After creating these files, we now have 5 + $special_count .txt files total
