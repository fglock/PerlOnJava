...
| Upstream suite coverage | **Public-module milestone delivered:** the four bundled thread distributions pass 64 files and 1,891 assertions in each backend/carrier configuration. **Core-wrapper milestone active:** the five non-regex core thread files pass 849/849 in all four backend/carrier modes; `make test-threads-core` requires the remaining twelve regex wrappers to complete after their same-commit direct companions. `make test-threads-ecosystem` adds pinned Test2, Storable, and Moose thread tests, a system-Perl DBI differential, Net::SSLeay 61/62, and DBIx::Class. Remaining direct regex-language gaps stay in the separate Phase 36 project. |
