...
- 🟡  **Perl Unicode Property Syntax**: Perl-specific properties execute through Joni. `Age`, cumulative `In`/`Present_In`, General_Category, Canonical_Combining_Class, Bidi_Class, Decomposition_Type, East_Asian_Width, Numeric_Value, Joining_Group, Block, Script, Script_Extensions, Grapheme_Cluster_Break, Sentence_Break, Word_Break, Line_Break, normalization Quick_Check, Hangul_Syllable_Type, Bidi_Paired_Bracket_Type, Indic_Conjunct_Break, Joining_Type, Numeric_Type, Vertical_Orientation, Indic_Positional_Category, Indic_Syllabic_Category, 51 core binary-property families, and the specialized `Hyphen`, `kEH_NoMirror`, `kEH_NoRotate`, `ID_Compat_Math_Continue`, `ID_Compat_Math_Start`, `IDSU`, and `MCM` families are generated from pinned Perl 5.44 Unicode 17.0 data with loose and wildcard aliases, ordered missing defaults, official compact aliases and shortcuts, Boolean complements, Script-versus-Script_Extensions policy, reserved or composite values, exact rationals, and Perl's generated decimal keyword aliases. Other generated property/value aliases still have pinned acceptance/rejection gaps.
...
- 🟡  **Extended Unicode Regex Features**: Complete pinned Script, Script_Extensions, Block, break-property, 51 core binary-property, seven specialized binary-property, QC/HST, five residual enumerated-property, and InPC/InSC sets, `Extended_Pictographic`, `Age`, `In`/`Present_In`, General_Category, Canonical_Combining_Class, Bidi_Class, Decomposition_Type, East_Asian_Width, Numeric_Value, Joining_Group, and invalid-property gates execute through Joni, and `regexp_unicode_prop.t` passes 1,110/1,110. Current generated evidence is 390,607/407,372 on both backends: chunks 01–04 pass 150,741/167,506 and the complete boundary corpus passes 239,866/239,866.
- 🟡  **Remaining generated Unicode property surface**: The measured combined map has 16,765 residual assertions, dominated by bare-alias/precedence and wildcard-dispatch cases, with smaller missing-data, diagnostic/value-policy, and five shared word-boundary modifier groups. Sixteen Line_Break surrogate deprecation-warning failures share TestProp source lines with passing membership assertions; counting source lines instead of emitted TAP assertions previously hid them. The diagnostic/value-policy category therefore contains 346 residual assertions. Identifier_Status/Identifier_Type and provisional `kEH_Core` data are separate active data slices.
...
- 🟡  **Regex Debugging**: Lexically scoped `use/no re 'debug'` and `debugcolor` are supported, including runtime snapshot ownership. Exact Perl optimizer/debug-transcript parity is not implemented; five `pat_re_eval.t` transcript assertions remain explicit non-semantic exclusions.
