...
use PerlOnJava::Phase36PerformanceEvidence ();

for my $bad ('9' x 200, '1e9', 'Inf', 'NaN', '1.', '.1' . ('2' x 30)) {
    ok(!PerlOnJava::Phase36PerformanceEvidence::number($bad),
        "numeric gate rejects adversarial token $bad");
}
ok(PerlOnJava::Phase36PerformanceEvidence::number('0.20'),
    'bounded ratified decimal is accepted');
is(PerlOnJava::Phase36PerformanceEvidence::canonical_decimal('0.20'), '0.2',
    'ratified decimal canonicalization occurs only after lexical validation');

my @issues;
PerlOnJava::Phase36PerformanceEvidence::validate_ratified_policy(\@issues, {
    thresholds => {
...
@issues = ();
my $parsed = PerlOnJava::Phase36PerformanceEvidence::parse_time(\@issues,
...
my $ok = eval { PerlOnJava::Phase36PerformanceEvidence::read_authority_key($key); 1 };
...
$ok = eval { PerlOnJava::Phase36PerformanceEvidence::read_authority_key($key); 1 };
