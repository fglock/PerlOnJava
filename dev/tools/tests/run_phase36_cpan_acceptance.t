...
my $tool = File::Spec->catfile($root, 'dev', 'tools', 'run_phase36_cpan_acceptance.pl');
...
print "ok 1 - $ENV{PHASE36_CPAN_TARGET} $ENV{PHASE36_CPAN_MODE}\n";
...
subtest 'checked-in policy contains the complete affected release set' => sub {
    my $checked = load_json(File::Spec->catfile($root, 'dev', 'tools',
...
subtest 'target-set drift and input mutation are fatal' => sub {
    ...
    my $moo_omission = load_json(File::Spec->catfile($root, 'dev', 'tools',
        'phase36_cpan_targets.json'));
    ...
    my $template_omission = load_json(File::Spec->catfile($root, 'dev', 'tools',
        'phase36_cpan_targets.json'));
