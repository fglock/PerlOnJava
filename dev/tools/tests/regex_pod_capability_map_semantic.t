...
my $tool = File::Spec->catfile($repository, 'dev', 'tools',
    'extract_regex_pod_inventory.pl');
my $checked_map = File::Spec->catfile($repository, 'dev', 'tools',
        ...
        families => [
            {
                id => 'recursion', status => 'implemented', evidence => [
                    { kind => 'source',
                      path => 'dev/tools/extract_regex_pod_inventory.pl',
                      note => 'fixture source evidence' },
                    { kind => 'test',
                      path => 'dev/tools/tests/regex_pod_capability_map_semantic.t',
