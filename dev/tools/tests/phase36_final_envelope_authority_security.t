...
my $tool = File::Spec->catfile($root, 'dev', 'tools', 'assemble_phase36_acceptance_envelope.pl');
my $requirements_template = File::Spec->catfile($root, 'dev', 'tools', 'phase36_acceptance_requirements.json');
...
subtest 'S1 rejects a skeletal A231/A232 graph' => sub {
    ...
    mutate_lane($f, 'performance', sub {
        ...
        $_[0]{authority} = {kind => 'phase36-performance-authority'};
    });
...
subtest 'S5 durable publication failure leaves no authority' => sub {
    ...
    write_file(File::Spec->catfile($lib,'Phase36SyncFailure.pm'), <<'MODULE');
package Phase36SyncFailure;
use strict;
use warnings;
use IO::Handle ();
...
MODULE
    ...
    local $ENV{PERL5OPT}='-MPhase36SyncFailure';
    my ($status,$log)=run_assembler($f);
    if ($status == 0) {
...
sub fixture {
    ...
    make_path(File::Spec->catdir($source_directory, 'dev', 'tools'));
    ...
    for my $side (qw(baseline candidate)) {
        $ordinary_artifacts{raw_logs}{$side} = [
            ...
            map { retained($directory, "ordinary-$side-$_.log",
                "PHASE36_REGEX_PERFORMANCE elapsed_seconds="
    ...
    my $performance_path = write_named_json($directory, 'final-performance.json', {
        ...
        authority => {schema_version=>1,kind=>'phase36-performance-authority',
