...
my $benchmark = File::Spec->catfile($root, 'dev', 'tools',
    'phase36_regex_benchmark.pl');
local $ENV{PHASE36_SOURCE_COMMIT} = '1' x 40;
local $ENV{PHASE36_JAR_SHA256} = '2' x 64;

my $output = qx{$^X "$benchmark" 2>&1};
is($? >> 8, 0, 'canonical ordinary-regex benchmark runs on system Perl');
my @metrics = grep { /^PHASE36_REGEX_PERFORMANCE\b/ } split /\r?\n/, $output;
