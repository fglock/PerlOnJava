...
my $original = File::Spec->catfile($repo, 'dev', 'tools',
...
subtest 'green fake make publishes sealed authoritative evidence' => sub {
    ...
    is($d->{schema}, 'perlonjava.phase36.make-evidence/v1', 'schema retained');
    is($d->{kind}, 'make', 'kind is make');
    is($d->{producer}, 'run_phase36_make_evidence.pl', 'producer is inert metadata');
...
sub fixture {
    my ($scenario, %arg) = @_;
    my $tmp = abs_path(tempdir('phase36-make-XXXXXX', TMPDIR => 1, CLEANUP => 1));
    ...
    write_file(File::Spec->catfile($source, '.phase36-fixture'),
...
GRADLEW
    ...
    run_ok([$git, '-C', $source, 'config', 'user.name', 'Phase36 Test']);
    run_ok([$git, '-C', $source, 'config', 'user.email', 'phase36@invalid.example']);
...
die "wrong make argv\\n" if \@ARGV;
die "wrong make cwd\\n" unless -f '.phase36-fixture' && -d '.git';
...
open my \$sf, '<', '.phase36-fixture' or die \$!;
...
my \$control = File::Spec->catfile(dirname(dirname(\$ARGV[1])), '.phase36-fixture');
