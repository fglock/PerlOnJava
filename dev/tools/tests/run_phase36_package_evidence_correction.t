...
my $temporary = tempdir('phase36-package-correction-XXXXXX', TMPDIR => 1,
    CLEANUP => 1);
my $commit = 'a' x 40;
...
subtest 'exact acceptance bridge retains only hashed durable artifacts' => sub {
    ...
    is($bridge->{producer}, 'run_phase36_package_evidence.pl',
...
subtest 'report mode publishes only a bounded non-acceptable failure record' => sub {
    ...
    is($failure->{kind}, 'phase36-package-evidence-failure',
...
sub fixture {
    ...
    make_path(File::Spec->catdir($source, 'dev', 'tools'),
...
JAVA
    for my $name (qw(verify-joni-distribution.pl verify-joni-packaging.pl)) {
        write_executable(File::Spec->catfile($source, 'dev', 'tools', $name),
            "#!/usr/bin/perl\nprint qq(ok\\n); exit 0;\n");
    }
    write_executable(File::Spec->catfile($source, 'dev', 'tools',
        'verify_phase36_notice_license.pl'), <<'NOTICE');
#!/usr/bin/perl
