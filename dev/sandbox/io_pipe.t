...
subtest 'Error handling and edge cases' => sub {
    subtest 'Invalid command handling' => sub {
        ...
        my $output_pipe_opened = open my $out_pipe, "| $invalid_cmd";
        ...
        my $input_pipe_opened = open my $in_pipe, "$invalid_cmd |";
        ...
        if ($input_pipe_opened) {
            ...
            ok(!$close_result || $exit_status != 0, 
               'Invalid command in input pipe should fail');
        } else {
    ...
    subtest 'Empty pipe handling' => sub {
        ...
        my $empty_cmd = $is_windows ? 'cmd /c ""' : 'true';
