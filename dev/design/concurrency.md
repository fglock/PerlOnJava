...
### Phase 6: threads Module (4 weeks)
...
6. `threads::shared` — SharedScalar/SharedArray/SharedHash wrappers
7. `lock()` — mutex on shared variables
8. `cond_wait()`/`cond_signal()`/`cond_broadcast()` — condition variables
9. `CLONE($package)` callback — called in child after cloning (Section 5.2)
10. Closure capture cloning with reference fixup (Section 5.1)
11. Method cache invalidation in child (Section 5.3)
...
|-------|----------|-------------|
| 0: Thread-Safety Audit | 2 weeks | Global lock safety |
| 1: PerlRuntime Shell | 4 weeks | ThreadLocal routing |
| 2: De-static I/O | 3 weeks | JSR-223 I/O isolation |
| 3: De-static CallerStack + DynamicScope | 4 weeks | Per-runtime call stacks |
| 4: De-static SpecialBlocks + Regex | 2 weeks | Per-runtime regex/blocks |
| 5: De-static GlobalVariable | 8 weeks | Full runtime isolation |
| 6: threads Module | 4 weeks | Perl `use threads` works |
| 7: Runtime Pool + JSR-223 | 3 weeks | Production web server |
| **Total** | **~30 weeks** | |

Phases 0-2 provide immediate value for JSR-223 embedding use cases.
Phase 5 is the critical path bottleneck (largest refactoring).
...
4. **`exit()` semantics:** In Perl 5 ithreads, `exit()` in a non-main thread
   exits only that thread (not the process). Need special handling.

5. **Reference fixup performance:** The two-pass clone (deep copy + identity
   map fixup) may be expensive for programs with many closures. Profile
   Perl 5's `perl_clone()` for comparison benchmarks.

6. **Nested closures:** A closure that captures a variable which is itself
