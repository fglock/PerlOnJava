...
die "Authority kind must be regex_implementation-envelope-authority\n"
    unless ($authority->{kind} // '') eq 'regex_implementation-envelope-authority';
...
die "Requirements gate set is not the Regex implementation ten-gate set\n"
...
die "Requirements gate kind map is not the exact Regex implementation policy\n"
...
sub producer_for {
    my ($gate) = @_;
    return 'run_regex_acceptance.pl'
        if $gate =~ /\A(?:ledger|jvm|interpreter)\z/;
    return 'collect_direct_thread.pl' if $gate eq 'direct-thread';
    return 'run_cpan_acceptance.pl' if $gate eq 'cpan';
    return 'run_final_performance.pl' if $gate eq 'performance';
    return 'run_package_evidence.pl' if $gate eq 'packaging';
    return 'verify_notice_license.pl' if $gate eq 'notice-license';
    return 'run_make_evidence.pl' if $gate eq 'make';
    return 'run_ci_evidence.pl' if $gate eq 'ci';
    ...
    die "Requirements performance authority contract is wrong\n"
        unless ($performance->{schema_version} // 0) == 1
            && ($performance->{authoritative_producer} // '') eq
                'dev/regex/tools/run_final_performance.pl'
            && ($performance->{authoritative_checker} // '') eq
                'dev/regex/tools/check_final_performance.pl'
...
sub validate_regex_release_authority {
    ...
    die "Regex release authority is not authoritative acceptance evidence\n"
        unless ($authority->{schema_version} // 0) == 1
            && ($authority->{kind} // '') eq 'regex_implementation-release-authority'
...
sub validate_cpan_execution_authority {
    ...
    die "CPAN execution authority schema is wrong\n"
        unless ($authority->{schema} // '') eq
            'perlonjava.regex_implementation.cpan-launch-authority/v1';
...
sub validate_performance {
    ...
    validate_ordered_performance_runs($document->{ordered}{runs},
        $final_identity, $performance_policy, $root);
    return {
        final_performance_contract => 'regex_implementation-final-performance/v1',
...
sub validate_cpan_mode {
    ...
    my @environment_keys = qw(PERLONJAVA_JAR PERLONJAVA_HOME HOME TMPDIR
        ...
        REGEX_IMPLEMENTATION_CPAN_TARGET REGEX_IMPLEMENTATION_CPAN_MODE);
    die "CPAN target $target $mode environment is wrong\n"
        unless ref($environment) eq 'HASH'
            ...
            && ($environment->{REGEX_IMPLEMENTATION_CPAN_TARGET} // '') eq $target
...
sub validate_prerequisite {
    ...
    die "Perl5 sync prerequisite producer is wrong\n"
        unless ($entry->{producer} // '') eq 'run_perl5_sync_evidence.pl';
    ...
    die "Perl5 sync prerequisite schema, kind, or status is wrong\n"
        unless ($document->{schema_version} // 0) == 1
            && ($document->{kind} // '') eq 'regex_implementation-perl5-sync-evidence'
            && ($document->{status} // '') eq 'pass';
...
sub validate_ci {
    ...
    die "CI producer schema or kind is wrong\n"
        unless ($document->{schema} // '')
                eq 'perlonjava.regex_implementation.final-envelope-bridge/v1'
            && count_number($document->{schema_version})
            && $document->{schema_version} == 1
            && ($document->{kind} // '') eq 'ci'
            && ($document->{producer} // '') eq 'run_ci_evidence.pl';
    ...
    die "CI retained evidence is not live exact-source evidence\n"
        unless ($evidence->{schema} // '')
                eq 'perlonjava.regex_implementation.ci-acceptance-evidence/v1'
...
sub validate_ci_policy {
    ...
    die "CI policy evidence is invalid\n"
        unless ($policy->{path} // '') eq 'dev/regex/tools/ci_evidence_policy.json'
            && ($policy->{requirements_path} // '')
                eq 'dev/regex/tools/acceptance_requirements.json'
...
sub validate_make {
    ...
    die "Make producer schema or mode is wrong\n"
        unless ($document->{schema} // '') eq
                'perlonjava.regex_implementation.make-evidence/v1'
...
sub wait_at_prepublication_test_boundary {
    return unless $ENV{REGEX_IMPLEMENTATION_ASSEMBLER_TEST_PREPUBLICATION_BOUNDARY};
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: assemble_acceptance_envelope.pl --authority FILE
...
then exclusively and atomically publish the schema-v1 Regex implementation acceptance
