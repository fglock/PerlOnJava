...
my @verifier_spec = (
    ...
    [notice_license => 'verify_notice_license.pl'],
);
my %verifier;
for my $spec (@verifier_spec) {
    my ($name, $file) = @$spec;
    my $path = safe_source_file($source, 'dev', 'regex', 'tools', $file);
...
my $work = tempdir('regex_implementation-package-evidence-XXXXXX', TMPDIR => 1, CLEANUP => 1);
my @commands;
my %command_log_path;
my %generated_files;
my %generated_trees;
...
my $report = {
    schema_version => 1,
    kind => 'regex_implementation-package-evidence-report',
...
if (!$strict_package_contract) {
    ...
    die "Legacy package evidence report must be explicitly non-authoritative\n"
        if $report->{authoritative} || exists $report->{completion}
            || $report->{kind} ne 'regex_implementation-package-evidence-report';
...
sub publish_evidence_bundle {
    ...
    my $stage = tempdir('.regex_implementation-package-stage-XXXXXX', DIR => $parent,
        CLEANUP => 1);
...
sub publish_failure_notice {
    ...
    my $document = {
        schema_version => 1, kind => 'regex_implementation-package-evidence-failure',
        producer => 'run_package_evidence.pl', mode => $option{mode},
...
sub test_fault {
    ...
    return ($ENV{PERLONJAVA_REGEX_IMPLEMENTATION_TEST_FAULT} // '') eq $name;
}
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: run_package_evidence.pl --source-root ABSOLUTE_CANONICAL_DIR
...
Produces final-freeze Regex implementation package evidence. The source must be a clean
