...
sub main {
    ...
    my $report = {
        schema_version => 1,
        kind => 'regex_implementation-release-manifest',
...
sub prepare_authority_inputs {
    my ($option) = @_;
    die "Regex implementation final performance checking is unsupported on Windows by the accepted A231 contract\n"
        if $^O eq 'MSWin32';
    my %trusted = %$option;
...
sub validate_final_performance_document {
    ...
    die "Final performance artifact schema is unsupported\n"
        unless ($document->{schema_version} // '') eq '1'
            && ($document->{kind} // '') eq 'regex_implementation-final-performance';
...
sub run_final_performance_checker {
    ...
    assert_authority_file_record($_) for values %{$trusted->{records}};
    return {
        contract => 'regex_implementation-final-performance/v1',
...
package RegexImplementationPinnedProgram;
BEGIN {
...
sub pin_validation_inputs {
    ...
    my @inputs = (
        [legacy => File::Spec->catfile($TOOL_DIR,
            'check_acceptance_manifest.pl'), 'legacy acceptance checker',
            File::Spec->catfile($root, 'check_acceptance_manifest.pl'),
            0500, 'dev/regex/tools/check_acceptance_manifest.pl'],
        [requirements => File::Spec->catfile($TOOL_DIR,
            'acceptance_requirements.json'), 'acceptance requirements',
            File::Spec->catfile($root, 'acceptance_requirements.json'),
            0400, 'dev/regex/tools/acceptance_requirements.json'],
        [verifier => File::Spec->catfile($TOOL_DIR,
            'verify_notice_license.pl'), 'strict notice-license verifier',
            File::Spec->catfile($root, 'verify_notice_license.pl'),
            0500, 'dev/regex/tools/verify_notice_license.pl'],
        [performance_checker => File::Spec->catfile($TOOL_DIR,
            'check_final_performance.pl'), 'final performance checker',
            File::Spec->catfile($root, 'check_final_performance.pl'),
            0500, 'dev/regex/tools/check_final_performance.pl'],
        [performance_module => File::Spec->catfile($TOOL_DIR, 'lib',
            ...
            0400, 'dev/regex/tools/lib/PerlOnJava/PerformanceEvidence.pm'],
        [performance_helper => File::Spec->catfile($TOOL_DIR,
            'JfrMetrics.java'), 'final performance JFR helper',
            File::Spec->catfile($root, 'JfrMetrics.java'),
            0400, 'dev/regex/tools/JfrMetrics.java'],
        [performance_orchestrator => File::Spec->catfile($TOOL_DIR,
            'run_final_performance.pl'), 'final performance producer',
            File::Spec->catfile($root, 'run_final_performance.pl'),
            0500, 'dev/regex/tools/run_final_performance.pl'],
        [performance_ordinary_producer => File::Spec->catfile($TOOL_DIR,
            'run_regex_performance.pl'), 'ordinary performance producer',
            File::Spec->catfile($root, 'run_regex_performance.pl'),
            0500, 'dev/regex/tools/run_regex_performance.pl'],
        [performance_benchmark => File::Spec->catfile($TOOL_DIR,
            'regex_benchmark.pl'), 'ordinary performance benchmark',
            File::Spec->catfile($root, 'regex_benchmark.pl'),
            0500, 'dev/regex/tools/regex_benchmark.pl'],
        [performance_schema => File::Spec->catfile($TOOL_DIR,
            'final_performance_schema.json'), 'final performance schema',
            File::Spec->catfile($root, 'final_performance_schema.json'),
            0400, 'dev/regex/tools/final_performance_schema.json'],
        [performance_assembler => File::Spec->catfile($TOOL_DIR,
            ...
            'final performance assembler', File::Spec->catfile($root,
                'assemble_final_performance.pl'),
            0500, 'dev/regex/tools/assemble_final_performance.pl'],
    );
    my %pinned;
    for my $input (@inputs) {
...
sub publish_atomic {
    ...
    my $stage_dir = tempdir('regex_implementation-stage-XXXXXXXX', DIR => $directory,
        CLEANUP => 0);
...
sub assert_atomic_link_capability {
    ...
    my $ok = eval {
        ...
        print {$fh} "regex_implementation-link-capability\n"
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: check_release_manifest.pl --evidence FILE
...
Final, fail-closed Regex implementation release wrapper. It preserves established notice and
