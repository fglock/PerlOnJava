...
my $source = $ENV{REGEX_IMPLEMENTATION_SOURCE_COMMIT} // ('0' x 40);
my $jar = $ENV{REGEX_IMPLEMENTATION_JAR_SHA256} // ('0' x 64);
die "REGEX_IMPLEMENTATION_SOURCE_COMMIT is malformed\n"
    unless $source =~ /\A[0-9a-f]{40}\z/;
die "REGEX_IMPLEMENTATION_JAR_SHA256 is malformed\n"
    unless $jar =~ /\A[0-9a-f]{64}\z/;

printf "REGEX_IMPLEMENTATION_REGEX_PERFORMANCE elapsed_seconds=%.9f throughput=%.3f checksum=%s jar_sha256=%s source_commit=%s\n",
