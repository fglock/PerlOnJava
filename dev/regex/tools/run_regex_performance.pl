...
sub execute_sample {
    ...
    if ($pid == 0) {
        ...
        %ENV = (%closed_environment,
            ...
            REGEX_IMPLEMENTATION_SOURCE_COMMIT => $identity->{source}{commit},
            REGEX_IMPLEMENTATION_JAR_SHA256 => $identity->{jar}{sha256},
            REGEX_IMPLEMENTATION_PERFORMANCE_SIDE => $side_name);
    ...
    die "$label metric prefix is malformed\n"
        unless shift(@tokens) eq 'REGEX_IMPLEMENTATION_REGEX_PERFORMANCE';
    my %metric;
    for my $token (@tokens) {
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: run_regex_performance.pl --baseline-source DIR --candidate-source DIR
...
bound to its exact JAR and PERLONJAVA_JAVA_BIN, and required to emit exactly one:
  ...
  REGEX_IMPLEMENTATION_REGEX_PERFORMANCE elapsed_seconds=N throughput=N checksum=TOKEN \
