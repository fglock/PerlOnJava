...
my %EXECUTABLE_PIN;
my %DATA_PIN;
my $REQUIREMENTS_FILE = 'dev/regex/tools/acceptance_requirements.json';
my $POLICY_FILE = 'dev/regex/tools/ci_evidence_policy.json';
my $PRODUCER_FILE = 'dev/regex/tools/run_ci_evidence.pl';
...
sub validate_policy {
    ...
    die "CI evidence policy kind is wrong\n"
        unless scalar_string($policy->{kind}, 'CI policy kind', 64)
            eq 'regex_implementation-ci-evidence-policy';
...
sub publish_atomic {
    ...
    my $dir = dirname($path);
    die "Refusing to overwrite output $path\n" if lstat($path);
    my $stage = tempdir('regex_implementation-ci-stage-XXXXXXXX', DIR => $dir, CLEANUP => 0);
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: run_ci_evidence.pl --source-dir ABS --git ABS [--gh ABS]
