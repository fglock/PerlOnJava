...
if ($strict_authority) {
    ...
    die "Package evidence does not identify its canonical producer\n"
        unless ($package->{producer} // '') eq 'run_package_evidence.pl';
    if ($producer{make}) {
...
sub validate_package_evidence {
    ...
    die "Retained package report is not the accepted strict relationship record\n"
        unless ($report->{schema_version} // 0) == 1
            && ($report->{kind} // '') eq 'regex_implementation-package-evidence-report'
...
sub authoritative_bridge {
    ...
    return {
        schema_version => 1,
        kind => 'regex_implementation-cpan-launch-bridge',
...
sub authority_marker {
    ...
    return {
        schema_version => 1,
        kind => 'regex_implementation-cpan-launch-authority',
        authoritative => JSON::PP::true,
...
sub publication_failpoint {
    my ($name) = @_;
    my $selected = $ENV{REGEX_IMPLEMENTATION_CPAN_BRIDGE_TEST_FAILPOINT} // '';
    die "Regex implementation CPAN bridge failpoint: $name\n" if $selected eq $name;
}

sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: prepare_cpan_launch_manifest.pl \
...
run_cpan_acceptance.pl. Every path must be canonical and absolute. The
