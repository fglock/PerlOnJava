...
usage(0) if $help;
die "Regex implementation checker process-tree and authority-key contract is unsupported on Windows until A232 validates native process trees and private fixed-location ACLs\n"
    if $^O eq 'MSWin32';
...
sub write_atomic_exclusive {
    ...
    my ($fh, $staging) = tempfile('.regex_implementation-check-XXXXXX',
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: check_final_performance.pl --evidence FINAL.json
...
Recompute Regex implementation performance status from every sealed raw artifact. Raw JFR
