...
use PerlOnJava::CpanJarSbom qw(decode_strict_json inspect_jar_sbom);

my %option = (
    policy => 'dev/regex/tools/cpan_targets.json',
    version_timeout => 30,
);
...
for my $target (@targets) {
    ...
    for my $mode (@{$target_policy->{required_modes}}) {
        ...
        my %environment = (
            ...
            JPERL_UNIMPLEMENTED => undef,
            REGEX_IMPLEMENTATION_CPAN_TARGET => $target,
        ...
        my $meta = {
            ...
            environment_sha256 => Digest::SHA::sha256_hex(canonical({ map {
                $_ => $environment{$_} } qw(PERLONJAVA_JAR PERLONJAVA_HOME HOME
                    ...
                    JPERL_UNIMPLEMENTED REGEX_IMPLEMENTATION_CPAN_TARGET
...
print "Regex implementation CPAN acceptance: $output\n";
exit($all_pass ? 0 : 1);
...
sub load_authority_bundle {
    ...
    die "Authority marker is not authoritative schema version 1\n"
        unless ($marker->{schema_version} // 0) == 1
            && ($marker->{kind} // '') eq 'regex_implementation-cpan-launch-authority'
    ...
    die "Authority bridge is not authoritative schema version 1\n"
        unless ($bridge->{schema_version} // 0) == 1
            && ($bridge->{kind} // '') eq 'regex_implementation-cpan-launch-bridge'
    ...
    for my $name (qw(package make)) {
        my $relative = "dev/regex/tools/run_${name}_evidence.pl";
...
sub validate_package_authority {
    ...
    die "Package authority marker is not an accepted canonical producer record\n"
        unless ($package->{schema_version} // 0) == 1
            ...
            && ($package->{producer} // '') eq 'run_package_evidence.pl'
    ...
    die "Retained package report producer is not canonical\n"
        unless ($report->{producer} // '') eq 'run_package_evidence.pl'
...
sub validate_make_authority {
    ...
    die "Make authority evidence is not an accepted canonical producer record\n"
        unless ($make->{schema} // '') eq 'perlonjava.regex_implementation.make-evidence/v1'
            && ($make->{kind} // '') eq 'make' && ($make->{mode} // '') eq 'acceptance'
            && ($make->{status} // '') eq 'pass'
            && ($make->{producer} // '') eq 'run_make_evidence.pl'
...
sub usage {
    print <<'USAGE';
Usage: run_cpan_acceptance.pl --authority-marker ABS --evidence-dir DIR [OPTIONS]

Run the immutable Regex implementation affected-CPAN target policy. Acceptance execution
takes only the canonical authority marker produced by
prepare_cpan_launch_manifest.pl and revalidates its launch, bridge,
...
Fake launchers may inspect REGEX_IMPLEMENTATION_CPAN_TARGET and REGEX_IMPLEMENTATION_CPAN_MODE. Every
