...
die "Regex implementation process-tree contract unsupported on Windows; A232 must provide and validate a native Windows tree authority\n"
    if $^O eq 'MSWin32';
...
my @forbidden_environment = qw(
    ...
    REGEX_IMPLEMENTATION_SOURCE_COMMIT
...
$ambient{$_} = 1 for grep {
    /\A(?:GIT|PERL|JAVA|JDK|CLASSPATH|JPERL|REGEX_IMPLEMENTATION)(?:_|\z)/
} keys %ENV;
...
for my $field (sort keys %trusted_path) {
    ...
    my $name = $field eq 'jfr_metrics_producer' ? 'JfrMetrics.java'
        : $field eq 'benchmark' ? 'regex_benchmark.pl'
        : $field eq 'ordinary_performance_producer'
            ? 'run_regex_performance.pl'
        : $field eq 'performance_evaluator' ? 'PerformanceEvidence.pm'
...
write_json_atomic($environment_contract_path, {
    ...
    lane_overrides => {
        ordinary => [qw(PERLONJAVA_JAR REGEX_IMPLEMENTATION_SOURCE_COMMIT
            REGEX_IMPLEMENTATION_JAR_SHA256 REGEX_IMPLEMENTATION_PERFORMANCE_SIDE)],
...
for my $index (0 .. $#order) {
    ...
    my $home = File::Spec->catdir($run_dir, '.regex_implementation-home');
    my $tmp = File::Spec->catdir($run_dir, '.regex_implementation-tmp');
    ...
    my $admission = {
        ...
        active_expensive_owners_before => ['regex_implementation-performance'],
        active_expensive_owners_after => ['regex_implementation-performance'],
...
$document->{authority} = seal_authority($document, $rules, {
    ...
    orchestrator => File::Spec->catfile($Bin, 'run_final_performance.pl'),
    ordinary_performance_producer => File::Spec->catfile($Bin,
        'run_regex_performance.pl'),
    performance_evaluator => File::Spec->catfile($Bin, 'lib', 'PerlOnJava',
        'PerformanceEvidence.pm'),
...
sub assert_no_competing_work {
    ...
    while (my $line = <$fh>) {
        next if $line =~ /run_final_performance/;
...
sub usage {
    my ($status) = @_;
    print <<'USAGE';
Usage: run_final_performance.pl
...
This is the only authoritative Regex implementation performance producer. It verifies the
