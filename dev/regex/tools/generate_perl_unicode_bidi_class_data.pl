...
my $unicore = select_unicode_root(
    ...
    required => [qw(version PropValueAliases.txt PropertyAliases.txt),
        File::Spec->catfile('extracted', 'DBidiClass.txt')],
);
...
my @sources = (
    {
        name => "DerivedBidiClass-$expected_version.txt",
        path => File::Spec->catfile($unicore, 'extracted', 'DBidiClass.txt'),
        hash => '4867b4b7f0731ed1bfcd34cc6251211ff1542541fce0734b6fbda139ee80b3a4',
        version => qr/^# DerivedBidiClass-\Q$expected_version\E\.txt$/m,
    },
    {
...
for my $line (split /\n/, $sources[1]{text}) {
    ...
    next unless @fields >= 3 && $fields[0] eq 'bc';
    ...
    for my $alias (@fields[1 .. $#fields]) {
        ...
        die "Bidi_Class alias collision for '$alias'\n"
            if exists $alias_index{$loose} && $alias_index{$loose} != $index;
        $alias_index{$loose} = $index;
    }
}
die "Expected 23 Bidi_Class values, found " . scalar(@short_values) . "\n"
    unless @short_values == 23;
...
for my $line (split /\n/, $sources[2]{text}) {
    ...
    $line =~ s/#.*$//;
    my @fields = map { trim($_) } split /;/, $line;
    next unless @fields >= 2 && ($fields[0] eq 'bc' || $fields[1] eq 'Bidi_Class');
    $property_aliases{loose_name($_)} = 1 for grep { length } @fields;
}
die "Pinned property aliases do not define bc and Bidi_Class\n"
    unless $property_aliases{bc} && $property_aliases{bidiclass};
...
for my $line (split /\n/, $sources[0]{text}) {
    if ($line =~ /^#\s*\@missing:\s*([0-9A-F]+(?:\.\.[0-9A-F]+)?)\s*;\s*([A-Za-z_]+)/) {
        ...
        die "Unknown \@missing Bidi_Class value '$value'\n" unless defined $index;
    ...
    my ($start, $end) = parse_range($range);
    my $index = $alias_index{loose_name($value)};
    die "Unknown explicit Bidi_Class value '$value'\n" unless defined $index;
    push @explicit, [$start, $end, $index];
}
die "Bidi_Class data has no ranges or defaults\n" unless @explicit && @missing;

my $default_index = $alias_index{loose_name('Left_To_Right')};
