...
my $source_tools = File::Spec->catdir($source, 'dev', 'regex', 'tools');
...
git_commit_files($source, 'canonical launch fixture', qw(jperl jcpan),
    'dev/regex/tools/run_package_evidence.pl',
    'dev/regex/tools/run_make_evidence.pl');
...
my $policy = File::Spec->catfile($root, 'dev', 'regex', 'tools',
    'cpan_targets.json');
...
sub write_authority {
    ...
    my $report = {
        schema_version => 1, kind => 'regex_implementation-package-evidence-report',
        producer => 'run_package_evidence.pl', mode => 'acceptance',
    ...
    write_json($package_path, {
        ...
        producer => 'run_package_evidence.pl', verified => JSON::PP::true,
