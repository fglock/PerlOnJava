...
my $test_file = "dev/regex/tools/tests/.regex_implementation-authority-$$-focused.t";
...
my $raw = File::Spec->catfile(File::Spec->tmpdir,
    "regex_implementation-authority-raw-$$-$backend.tap");
open my $raw_fh, '>:raw', $raw or die $!;
print {$raw_fh} "1..1\nok 1 - fixture\n";
close $raw_fh;
open my $ofh, '>:raw', $output or die $!;
print {$ofh} JSON::PP->new->encode({results => {$file => {
...
sub evidence_fixture {
    ...
    if ($change{package_report}) {
        $package_document->{kind} = 'regex_implementation-package-evidence-report';
