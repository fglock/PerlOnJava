...
my $requirements = $ENV{REGEX_IMPLEMENTATION_ACCEPTANCE_REQUIREMENTS}
    // File::Spec->catfile($root, 'dev', 'regex', 'tools',
...
my $legacy_checker = $ENV{REGEX_IMPLEMENTATION_ACCEPTANCE_CHECKER}
    // File::Spec->catfile($root, 'dev', 'regex', 'tools',
...

{
    ...
    my $path = File::Spec->catfile($fixture->{directory}, $lane->{artifact}{path});
    my $record = read_json($path);
    $record->{schema} = 'perlonjava.regex_implementation.make-evidence-report/v1';
...

{
    ...
    my $path = File::Spec->catfile($fixture->{directory}, $lane->{artifact}{path});
    my $record = read_json($path);
    $record->{kind} = 'regex_implementation-package-evidence-report';
    $record->{authoritative} = JSON::PP::false;
    write_json($path, $record);
    $lane->{artifact}{sha256} = sha_file($path);
    rewrite_authority($fixture);
...
sub fixture {
    ...
    $performance_document->{authority} = {
        schema_version => 1, kind => 'regex_implementation-performance-authority',
