...
} qw(run_final_performance.pl run_regex_performance.pl
    check_final_performance.pl assemble_final_performance.pl);
my $module = read_raw(File::Spec->catfile($tools, 'lib', 'PerlOnJava',
    'PerformanceEvidence.pm'));

for my $tool (qw(git ps uptime)) {
    like($source{'run_final_performance.pl'}, qr/'\Q$tool\E=s'/,
        "producer requires authority-selected $tool");
    like($source{'check_final_performance.pl'}, qr/'\Q$tool\E=s'/,
        "checker accepts authority-selected $tool");
}
like($source{'run_regex_performance.pl'}, qr/'git=s'/,
    'ordinary nested producer requires authority-selected Git');
unlike($source{'run_final_performance.pl'}, qr/'ordered-test=s'/,
    'ordered test cannot be replaced through the CLI');
like($source{'run_final_performance.pl'},
...
for my $text ($source{'run_final_performance.pl'},
        $source{'run_regex_performance.pl'}, $module) {
...
like($source{'run_final_performance.pl'}, qr/IO::Select.*sysread/s,
...
like($source{'run_regex_performance.pl'}, qr/IO::Select.*sysread/s,
...
like($source{'run_final_performance.pl'}, qr/sub copy_tree.*validate_fixture_tree/s,
    'fixture tree is validated before copying');
like($source{'run_final_performance.pl'}, qr/fixture symlinks are forbidden/,
    'fixture symlinks are rejected');
like($source{'run_final_performance.pl'}, qr/exceeds depth 32/,
    'fixture depth is bounded');
like($source{'run_final_performance.pl'}, qr/exceeds 20000 entries/,
    'fixture entry count is bounded');
like($source{'run_final_performance.pl'}, qr/file exceeds 256 MiB/,
    'fixture per-file bytes are bounded');
like($source{'run_final_performance.pl'}, qr/exceeds 2 GiB aggregate/,
...
like($source{'run_final_performance.pl'}, qr/PATH => ''/,
...
my $schema = JSON::PP->new->decode(read_raw(File::Spec->catfile($tools,
    'final_performance_schema.json')));
my $requirements = JSON::PP->new->decode(read_raw(File::Spec->catfile($tools,
    'acceptance_requirements.json')))->{performance_acceptance};
is($schema->{kind}, 'regex_implementation-final-performance', 'schema kind matches implementation');
is($schema->{authority}{kind}, 'regex_implementation-performance-authority',
