...
my $verifier = File::Spec->catfile(
    $repository, 'dev', 'regex', 'tools', 'verify_notice_license.pl');
...
my $requirements_path = File::Spec->catfile(
    $repository, 'dev', 'regex', 'tools', 'acceptance_requirements.json');
...
my $checker = File::Spec->catfile(
    $repository, 'dev', 'regex', 'tools', 'check_acceptance_manifest.pl');
is(system($^X, $checker,
