...
subtest 'successful report is permanently non-authoritative' => sub {
    ...
    my $d = decode_json(read_file($f->{output}));
    is($d->{schema}, 'perlonjava.regex_implementation.make-evidence-report/v1',
...
subtest 'CLI is exact, case-sensitive, non-abbreviating, and duplicate-free' => sub {
    for my $extra (
        ...
        ['--unknown-regex_implementation', 'x'], ['--mode', 'acceptance'],
    ) {
        my $f = capture_fixture('make_fail');
...
sub capture_fixture {
    my ($scenario, %arg) = @_;
    my $tmp = abs_path(tempdir('regex_implementation-capture-reducer-XXXXXX',
        TMPDIR => 1, CLEANUP => 1));
...
print {\$jar_fh} "regex_implementation fake jar\\n\$embedded\\n"; close \$jar_fh;
}
...
exit 0;
    ...
    run_ok([$git, '-C', $source, 'config', 'user.name', 'RegexImplementation Reducer']);
    run_ok([$git, '-C', $source, 'config', 'user.email',
        'regex_implementation-reducer@invalid.example']);
    ...
    if ($scenario eq 'zip_green' || $scenario eq 'zip_wrong') {
        ...
        $jar_bytes = "regex_implementation fake jar\n$embedded\n";
    }
...
sub consumer_accepts {
    ...
    return 0 unless $d->{schema} eq 'perlonjava.regex_implementation.make-evidence/v1'
