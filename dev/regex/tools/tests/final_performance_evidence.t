...
my $checker = File::Spec->catfile($root, 'dev', 'regex', 'tools',
    'check_final_performance.pl');
my $orchestrator = File::Spec->catfile($root, 'dev', 'regex', 'tools',
...
write_raw($authority_key, 'regex_implementation-test-authority-key-' . ('x' x 64));
...
subtest 'oversized decimals and incomplete GC pairing fail before arithmetic' => sub {
    ...
    my $evaluation = evaluate_performance($document, $bad_rules, $dir, {
        ...
        ordinary_performance_producer => File::Spec->catfile($root, 'dev', 'regex', 'tools', 'run_regex_performance.pl'),
        performance_evaluator => File::Spec->catfile($root, 'dev', 'regex', 'tools',
            'lib', 'PerlOnJava', 'PerformanceEvidence.pm'),
        benchmark => File::Spec->catfile($root, 'dev', 'regex', 'tools',
            'regex_benchmark.pl'),
        jfr_metrics_producer => File::Spec->catfile($root, 'dev', 'regex', 'tools',
            'JfrMetrics.java'), requirements => $requirements,
    });
...
subtest 'atomic assembler publication never exposes a partial final file' => sub {
    ...
    my @staging = glob(File::Spec->catfile($dir, '.regex_implementation-final-*'));
...
sub fixture {
    ...
    my $helper_source = read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools',
        'JfrMetrics.java'));
    $identity{benchmark} = artifact($dir, 'sealed/regex_benchmark.pl',
        read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools',
            'regex_benchmark.pl')));
    $identity{ordinary_performance_producer} = artifact($dir,
        'sealed/run_regex_performance.pl',
        read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools',
            'run_regex_performance.pl')));
    $identity{performance_evaluator} = artifact($dir,
        'sealed/PerformanceEvidence.pm',
        read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools', 'lib',
    ...
    my @cleared = qw(JPERL_OPTS JPERL_UNIMPLEMENTED PERL_SKIP_PSYCHO_TEST
        ...
        PERL_MM_OPT GIT_DIR GIT_WORK_TREE GIT_INDEX_FILE REGEX_IMPLEMENTATION_SOURCE_COMMIT
        REGEX_IMPLEMENTATION_JAR_SHA256 REGEX_IMPLEMENTATION_PERFORMANCE_SIDE);
    $identity{execution_environment} = artifact($dir,
        'sealed/execution-environment.json', $json->encode({
            ...
            base_effective_environment => {
                ...
                HOME => '/private/regex_implementation/home',
                PERLONJAVA_HOME => '/private/regex_implementation/home',
                TMPDIR => '/private/regex_implementation/tmp',
    ...
    $identity{jfr_metrics_producer} = artifact($dir,
        'sealed/JfrMetrics.java', $helper_source);
    ...
    for my $side (qw(baseline candidate)) {
        ...
        for my $index (0 .. $#seconds) {
            ...
            push @logs, artifact($dir, "ordinary/$side-$index.log",
                "REGEX_IMPLEMENTATION_REGEX_PERFORMANCE elapsed_seconds=$elapsed throughput="
                . (72000 / $elapsed)
    ...
    if ($mode eq 'huge-ordinary') {
        ...
        substitute_artifact($dir, $log,
            "REGEX_IMPLEMENTATION_REGEX_PERFORMANCE elapsed_seconds=$huge throughput=1"
    ...
    for my $index (0 .. $#order) {
        ...
        my $run = {
            ...
            time_raw => artifact($dir, "$prefix.time",
                ($mode eq 'timing-summary' && $candidate_side && $index == 1)
                    ? "REGEX_IMPLEMENTATION_TIME wall_seconds=1 user_seconds=1 system_seconds=0 max_rss_bytes=1\n"
                    : $mode eq 'gnu-time'
        ...
        my @owners = ('regex_implementation-performance');
        ...
        my $environment = {
            ...
            private_roots => {
                HOME => "/private/regex_implementation/$index/home",
                PERLONJAVA_HOME => "/private/regex_implementation/$index/perlonjava",
                TMPDIR => "/private/regex_implementation/$index/tmp",
            },
            effective_environment => {
                ...
                PERLONJAVA_HOME => "/private/regex_implementation/$index/perlonjava",
                TMPDIR => "/private/regex_implementation/$index/tmp",
                PERLONJAVA_JAR => '/private/regex_implementation/sealed/candidate.jar',
...
sub trusted_repositories {
    ...
    git_ok($candidate, 'config', 'user.email', 'regex_implementation-test@example.invalid');
    git_ok($candidate, 'config', 'user.name', 'RegexImplementation Test');
    ...
    for my $name (qw(run_final_performance.pl
            run_regex_performance.pl regex_benchmark.pl
            JfrMetrics.java)) {
        my $target = File::Spec->catfile($candidate, 'dev', 'regex', 'tools', $name);
        make_path(dirname($target));
        write_raw($target, read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools', $name)));
    }
    my $evaluator = File::Spec->catfile($candidate, 'dev', 'regex', 'tools', 'lib',
        'PerlOnJava', 'PerformanceEvidence.pm');
    make_path(dirname($evaluator));
    write_raw($evaluator, read_raw(File::Spec->catfile($root, 'dev', 'regex', 'tools',
        'lib', 'PerlOnJava', 'PerformanceEvidence.pm')));
    ...
    make_path($perl5);
    git_ok($perl5, 'init', '-q');
    git_ok($perl5, 'config', 'user.email', 'regex_implementation-test@example.invalid');
    git_ok($perl5, 'config', 'user.name', 'RegexImplementation Test');
