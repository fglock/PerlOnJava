...
sub fixture {
    my $tmp = abs_path(tempdir('regex_implementation-durability-XXXXXX',
        TMPDIR => 1, CLEANUP => 1));
...
print {\$jar_fh} "regex_implementation durability jar\\n\$commit\\n"; close \$jar_fh;
...
exit 0;
    ...
    run_ok([$git, '-C', $source, 'config', 'user.name', 'RegexImplementation Durability']);
    run_ok([$git, '-C', $source, 'config', 'user.email',
        'regex_implementation-durability@invalid.example']);
    run_ok([$git, '-C', $source, 'add', '-A']);
    run_ok([$git, '-C', $source, 'commit', '-q', '-m', 'durability fixture']);
    my $commit = capture([$git, '-C', $source, 'rev-parse', 'HEAD']);
    chomp $commit;
    my $jar_bytes = "regex_implementation durability jar\n$commit\n";
...
sub run_capture {
    ...
    if ($pid == 0) {
        if (defined $failpoint) {
            $ENV{REGEX_IMPLEMENTATION_MAKE_EVIDENCE_FAILPOINT} = $failpoint;
        } else {
            delete $ENV{REGEX_IMPLEMENTATION_MAKE_EVIDENCE_FAILPOINT};
        }
