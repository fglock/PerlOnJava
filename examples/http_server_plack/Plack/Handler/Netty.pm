...
sub new {
    ...
    my $self = bless {
        ...
        port             => $args{port}             || 5000,
        backlog          => $args{backlog}          || 128,
        keepalive        => $args{keepalive}        || 30,
        max_request_size => $args{max_request_size} || 10485760,  # 10MB default
    }, $class;
...
sub run {
    ...
    unless ($app && ref($app) eq 'CODE') {
        die "Plack::Handler::Netty->run() requires a PSGI application coderef\n";
    }

    # Create and start the Netty server
    # The Java class is available directly via the package name
    # (PerlOnJava's class loader finds it automatically)
    my %config = (
        host             => $self->{host},
    ...
    eval {
        my $server = examples::http_server_plack::NettyPSGIServer->new(
            $self->{port},
            $app,
            \%config
        );

        print STDERR "Netty PSGI Server starting on $self->{host}:$self->{port}\n";
    ...
    if ($@) {
        die "Failed to start Netty server: $@\n";
    }
}
