...
foreach my $i (0..$#left_ops) {
    ...
    print "not " if $flipflop != ($i == 1 || $i == 2 || $i == 3);
    say "ok # Flip-flop using two dots at iteration $i";
}
...
foreach my $i (0..$#three_dot_left_ops) {
    ...
    print "not " if $three_dot_flipflop != ($i == 1 || $i == 2);
    say "ok # Flip-flop using three dots at iteration $i";
}
...
foreach my $i (0..$#left_seq_ops) {
    ...
    print "not " if $seq_flipflop != ($i == 1 ? 1 : $i == 2 ? 2 : $i == 3 ? "3E0" : "");
    say "ok # Sequence number generation at iteration $i";
}
...
foreach my $i (0..$#deferred_left_ops) {
    ...
    print "not " if $deferred_flipflop != ($i == 1 ? 1 : $i == 2 ? 2 : "");
    say "ok # Deferred right operand in three dots at iteration $i";
}
