...
description: Publish the five generated PerlOnJava CPAN compatibility report files through a lock-consistent commit, GitHub PR, and merge. Use when asked to save, publish, or submit current dev/cpan-reports compatibility results; do not use for editing tester code or classifying failures.
---
...
- `dev/cpan-reports/cpan-compatibility-perl-fail.dat`
...
tree pre-flight before proceeding. Stage only the five paths above; never use
...
exclusive `flock` on that file, run `git add` for all five report paths while
holding the lock, and release it only after `git add` exits.

The tester holds this same lock while atomically replacing all five files, so
...
- the staged diff contains exactly the five report paths;
- the line counts of the pass, fail, and skip `.dat` files equal the pass, fail,
  and skipped values in the staged Markdown summary, while the Perl-failure
  `.dat` count equals the skipped-because-Perl-failed value;
...
after verifying that it is open and contains exactly the five expected files.
